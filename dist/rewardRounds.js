...
if (env_1.default.chain == "eos") { contract = "powerup.nfts" }
...
async function claimAll() {
  ...
  for (const row of unclaimed) {
    ...
    if (!result) { console.error("claim erorr", row) } else {
      if (result.receipts.length == 0) { console.error("claim erorr:", result.errors) } else {
...
async function init() {
  try {
    ...
    for (const round of unrewarded) {
      ...
      if (!result) { console.error("rewardround erorr", round) } else {
        if (result.receipts.length == 0) { console.error("rewardround erorr:", result.errors) } else {
          console.log(result.receipts[0].url.toString())
          console.log(result.receipts[0].receipt.id)
        }
      }
